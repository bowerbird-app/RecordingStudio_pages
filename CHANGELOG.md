...
- Hero edit groups look under **Style**. **Preset** is On a dark photo or On a light photo, and only shows for a fullscreen picture. **Headline** and **Quieter line** are Flatpack ColorSwatch overrides (`title_color`, `body_color`). Unused settings still save.
...
### Upgrade notes
- Pin Flatpack `v0.1.186` (or later). Drop any `h-dvh` wrap around Hero. On a fullscreen image hero, set `--hero-overlay-min-height: 100dvh` on the section. Hero **Align** is Center or Left (`align:`). **Preset** On a dark photo / On a light photo maps to `on:` and only belongs on a fullscreen picture. Optional hex `title_color` / `body_color` override overlay or surface text tokens. Right align is not valid. Overlay copy sits high in the frame, not vertically centered. Menu frost is kit-owned; do not attach a window scroll listener. If you overrode the section form, take the Style group and gated preset.
