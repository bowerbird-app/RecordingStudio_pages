...
Workspace (root recording)
  └── Page recording (title, homepage)
        ├── Section recording (section_type: hero, content, settings)
        │     └── Attachment children (photos for that section)
        ├── Section recording (section_type: feature_grid, title, intro, layout)
        │     └── Section recording (section_type: feature, title, body)
        │           └── Attachment children (that feature's picture)
        ├── Section recording (section_type: logo_cloud, title)
        │     └── Section recording (section_type: logo, name, url, image)
...
`fields.title` may be `:string` or `{ type: :string, required: true }`. `recording_ids` stores Recording ids. Resolve the actual records at render time, or with `data:`. Do not copy domain records into section JSON. `:attachment` stores an Attachable child id on the section. Built-in hero, image-and-text, feature, and logo use `image`. Old `image_url` values upgrade on read.

`child_types:` lists the section keys that may sit under this type. Feature grid lists `feature`. Logo cloud lists `logo`. A page only accepts types that are not anyone's child. `AddSection` enforces that: pass `page_recording:` for a page section, or `parent_recording:` for a child section. One level. A feature does not contain another section.
...
`RecordingStudioPages.catalog` lists every registered section and call to action, including field types, required flags, settings, and variants. That catalog is the machine-readable surface for future API and MCP tooling.

## Register a call to action

A hero has one slot under the copy. Pages fills it from a **CTA registry**, not by forking the hero. The built-in filling is a button (`text` + `url`). Hosts and other gems register more fillings — social logins, a URL field, a waitlist form — and pick them on the hero.
...
Rich text is a full-width card with a display headline, muted body, and Hero inset (`px-16 py-24`). Copy stays `max-w-xl` so a wide card does not turn into a newspaper column. Background is Default, Muted, or Inverted. An optional corner image uses the same Attachable `image` field as hero; skip it and the card is words only. When the image is present, the card keeps extra space under the words so the mark stays in the corner. Narrow keeps a reading-width card. Copy is JSON plus `sanitize`. Action Text expects a mutable record, so this gem does not use `has_rich_text`. Hero, image-and-text, feature, logo, menu, and rich-text photos are Attachable children of that section. The JSON stores the attachment id; public render turns it into an Active Storage path.

Dummy seeds published **Tonight**, **Join**, **Walk in**, and **Start from a URL** pages. Open Tonight at `/pages/:uuid/tonight`, Join at `/pages/:uuid/join`, Walk in at `/pages/:uuid/walk-in`, and the URL landing at `/pages/:uuid/start-from-a-url`. Those public URLs are the page, not the editor preview. A fullscreen hero fills the first viewport with `--hero-overlay-min-height: 100dvh`. Dummy Home starts with a menu, a left-aligned hero, logos, features, and a CTA. Seed restores those sections if they drift, then points the Home menu at the seeded About, Tonight, and Join pages.

List fields skip blank extra slots and items marked `_destroy`.

## Admin

RS Admin gets a Pages section. The hub’s primary action is **Page** (plus icon, then the word Page). It opens the new-page form at `/recording_studio_pages/admin/pages/new`. That form is a reading-width column (`max-w-xl`). **Create page** is compact, not full width. Unticked “Use as the public home page” stores false (an unchecked box does not submit). **View all** opens the Admin list at `/admin/screens/pages`. The nested editor still lives at `/recording_studio_pages/admin/pages/:id` because RS Admin is a hub of screens and widgets, not a nested recording editor. The editor lists sections in a padded Flatpack Card around an orderable list with no decimal markers. Each row shows Flatpack List’s `arrows-up-down` icon, then the section type name as a link to edit that section. Hover uses the List item background. Drag a row to change order. Copy, edit, turn off, and remove live in the row’s three-dot menu. Copy uses Recording Studio Duplicatable (`duplicate_in_place!`) so the new row is another generic section recording under the same page, then Orderable `recording_studio_orderable_append!` puts it at the end.

The editor is a two-column Flatpack Grid below the action row: the section list on the left, the live page on the right. The live column has no heading. Small screens stack those columns. Enabled sections use the same components as the public page. Unpublished pages stay private on public routes. Add a section from **Section** (plus icon; each type has an icon). The action row is **Section**, Publishable’s Draft/Published control, then **Settings** (cog dropdown). Open **Settings** in that menu to rename or set home. **Trash** uses Recording Studio Trashable. A section type name in the list (Hero, Menu, and so on) opens that section. The Draft / Published control is Publishable’s `render_publishable_quick_actions` dropdown (scheduled pages show the date). An empty list uses Empty State **Add your first section**. An empty live column uses Empty State **Preview**.

Flashes live in one `#flash` slot on the layout that wraps page builder screens (`recording_studio_pages_flash`). Create, save, and remove still redirect with a Rails flash. **Section** replaces that slot over Turbo so the last message wins. Do not put a second notice inside the editor. Skip the helper on layouts that also wrap RS Admin — isolated Admin controllers do not include it.

Edit section puts **Update** and **Cancel** under the title, then the same Grid: the form on the left, that one section on the right. A feature grid or logo cloud keeps its own fields, then the same orderable list the page uses. One child type shows a single **Feature** or **Logo** button. The row name is the feature title or the logo name. Copy fields sit first, then a **Call to action** divider if the section has a CTA, then Layout. Hero also has a **Style** group: Align, Eyebrow, Headline and Subtitle colour swatches, and **Preset** only when Layout is a fullscreen photo with a picture. The buttons are compact, not full width. Update stays on the section and refreshes the preview. Cancel goes back to the page. The preview column has no heading. It uses the same components as the public page, including a section that is turned off. Small screens stack those columns.

Gem screens call `recording_studio_pages_nav`, which uses Recording Studio page nav (back and close). Pass the original trigger as `anchor_url` so Close can leave the action stack. That is the root that opened Admin, not the Pages list. Dummy default layout maps `page_nav_anchor_url` to Flatpack PageNav `anchor_href`. Dummy signed-in `/` uses a Flatpack sidebar and a root switcher in the top bar. Dummy `/docs` still adds a root switcher and Sign out through `dummy_page_nav`. Do not wrap that host chrome onto gem screens.
...
The hub’s Published and Drafts cards load through Turbo frames. Each card opens the Pages list. Drafts arrives with Draft selected. Published arrives with Published selected. Dummy imports `@hotwired/turbo-rails` and pins RS Admin Stimulus (`controllers/recording_studio_admin`, the same pin `recording_studio_admin:install` adds). Without those, the cards stay on the shimmer. Hosts that already ran the Admin install already have the Stimulus pin; they still need Turbo on the layout that serves `/admin`.

The Pages list can filter by Publishable status (Draft, Scheduled, Published) and by home page (Home or Other pages). Columns are Page, Home, Updated at, then Status, then the row menu. Dummy puts **Page** (plus icon) under the title, not in the page-nav right slot. A live page name uses Publishable’s public View link and opens in a new tab (`target="_blank"`, `data-turbo="false"`) so Admin’s table frame does not swallow it. Draft and scheduled names stay plain text. Status on each row is Publishable’s Draft / Scheduled / Published control (`render_publishable_quick_actions`). Isolated Admin gets that helper plus Publishable’s mounted routes so the control can publish from the list. The last column is a row menu: **Edit** opens the page editor, **Trash** removes the page. Hub **Page**, list **Page**, and **Edit** keep the incoming `anchor_url` from the original trigger (dummy’s signed-in home passes `/`) so Close returns there instead of the Pages list.
