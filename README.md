...
Gem screens call `recording_studio_pages_nav`, which uses Recording Studio page nav (back and close). Pass the original trigger as `anchor_url` (query or the helper’s default) so Close can leave the action stack. Dummy default layout maps `page_nav_anchor_url` to Flatpack PageNav `anchor_href`. That default layout stays host-agnostic. Dummy signed-in `/` uses a Flatpack sidebar and a root switcher in the top bar. Dummy `/docs` still adds a root switcher and Sign out through `dummy_page_nav`. Do not wrap that host chrome onto gem screens.
