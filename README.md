...
The Pages list can filter by Publishable status (Draft, Scheduled, Published) and by home page (Home or Other pages). Columns are Page, Home, Updated at, then Status, then the row menu. Dummy puts **Page** (plus icon) under the title, not in the page-nav right slot. A live page name in the first column links to the published URL. Status on each row is Publishable’s Draft / Scheduled / Published control (`render_publishable_quick_actions`). Isolated Admin gets that helper plus Publishable’s mounted routes so the control can publish from the list. The last column is a row menu: **Edit** opens the page editor, **Trash** removes the page.
