...
- RS Admin Pages section at `/admin`. Switch to the Admin root first. **Page** (plus icon, then the word Page) opens the new-page form. **View all** opens the Admin list of pages. That list puts **Page** under the title, not in the top-right slot. Columns are Page, Home, Updated at, Status, then the row menu. A live page name links to the published URL. The list also has a **Status** filter (Draft, Scheduled, Published), a **Home page** filter, Publishable’s status control on each row, and an actions menu with Edit and Trash. The hub also lists **Users**. Dummy loads Turbo and RS Admin Stimulus so Published and Drafts leave the shimmer, show counts, and open the Pages list. Drafts opens that list with Draft selected. Dummy draws those hub buttons with Flatpack `href:` so they navigate.
