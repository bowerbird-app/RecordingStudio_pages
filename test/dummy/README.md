...
- Staff page composition at `/recording_studio_pages/admin/pages`. Add a section from the editor dropdown. Use a template. Edit the section list in a padded Card in the first column and see enabled sections in the second. Open a section for **Update** and **Cancel** under the title, the form on the left, and a live preview on the right. Update stays on that section. Hero edit offers **Align** (Center or Left) and **Photo** (Dark or Light). **Choose image** on hero, image-and-text, logo items, and rich text opens the Attachable picker for that section. Drag sections to reorder. Copy a section from the row’s three-dot menu (Recording Studio Duplicatable; photos come along). Remove a page from Edit page. Dummy default layout keeps one `#flash` slot (`recording_studio_pages_flash`). Add section and Use a template replace that slot instead of stacking a second notice in the editor.
