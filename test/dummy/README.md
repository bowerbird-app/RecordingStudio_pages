...
- `/admin` - RS Admin hub. Switch the current root to **Admin** first. **Page** goes to `/recording_studio_pages/admin/pages/new` (reading-width form, compact **Create page**). **View all** goes to `/admin/screens/pages`. Live pages and Drafts open that list; Drafts adds `status=Draft`. The hub returns 403 while a workspace is selected. Live pages and Drafts load through Turbo frames; dummy pins Turbo and RS Admin Stimulus so those cards leave the shimmer.
