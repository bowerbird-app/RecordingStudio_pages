...
class CursorSkillFetchTest < ActiveSupport::TestCase
  test "fetch-skills extras come from the plugin catalog without hardcoded extra URLs" do
    root = RecordingStudioPages::Engine.root
  ...
  test "gem version stays 0.3.4 and gemspec still excludes .cursor" do
    assert_equal "0.3.4", RecordingStudioPages::VERSION

    spec = Gem::Specification.load(RecordingStudioPages::Engine.root.join("recording_studio_pages.gemspec").to_s)
    cursor_files = spec.files.select { |path| path == ".cursor" || path.split("/").include?(".cursor") }

    assert_empty cursor_files

    tracked = `git -C #{RecordingStudioPages::Engine.root} ls-files -- .cursor/rules`
