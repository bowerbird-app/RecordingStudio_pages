...
require "fileutils"
require "tmpdir"
require "generators/recording_studio_pages/install/install_generator"

class InstallGeneratorTest < Minitest::Test
  INSTALL_TEMPLATE_PATH = File.expand_path(
    "../lib/generators/recording_studio_pages/install/templates/INSTALL.md",
    __dir__
  )
  ...
  def build_generator(destination_root, options = {})
    RecordingStudioPages::Generators::InstallGenerator.new(
      [],
      options,
  ...
  def test_mount_engine_uses_configured_mount_path
    ...
    assert_equal ["mount RecordingStudioPages::Engine, at: \"/addons/recording\""], routes
  end
  ...
  def test_install_guide_includes_migration_and_host_setup_steps
    ...
    assert_includes install_guide, "bin/rails generate recording_studio_pages:migrations"
    assert_includes install_guide, "bin/rails db:migrate"
    assert_includes install_guide, "auth, layout, and current actor"
    assert_includes install_guide, "recording_studio_pages_flash"
    assert_includes install_guide, "publishable_preview_badge"
