...
class EngineTest < Minitest::Test
  def setup
    @original_configuration = RecordingStudioPages.instance_variable_get(:@configuration)
  ...
  def teardown
    ...
    RecordingStudioPages.instance_variable_set(:@configuration, @original_configuration)
  end
  ...
  def test_before_and_after_initialize_initializers_run_hooks
    ...
    RecordingStudioPages.configuration.hooks.before_initialize { |_engine| before_called = true }
    RecordingStudioPages.configuration.hooks.after_initialize { |_engine| after_called = true }

    find_initializer("recording_studio_pages.before_initialize").block.call(Object.new)
    find_initializer("recording_studio_pages.after_initialize").block.call(Object.new)
  ...
  def test_load_config_merges_config_sources_and_runs_on_configuration_hook
    hook_called = false
    hook_payload = nil
    RecordingStudioPages.configuration.hooks.on_configuration do |cfg|
      hook_called = true
      hook_payload = cfg
    end

    xcfg = Struct.new(:recording_studio_pages).new({ register_built_in_sections: false })
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { homepage_path: "/welcome", page_parent_types: %w[Workspace] }
      end
    end.new(app_config)

    find_initializer("recording_studio_pages.load_config").block.call(app)

    assert hook_called
    assert_equal RecordingStudioPages.configuration, hook_payload
    assert_equal "/welcome", RecordingStudioPages.configuration.homepage_path
    assert_equal %w[Workspace], RecordingStudioPages.configuration.page_parent_types
    assert_equal false, RecordingStudioPages.configuration.register_built_in_sections
  end

  def test_load_config_handles_errors_and_each_pair_fallback
    pair_config = Class.new do
      def each_pair
        yield(:homepage_path, "/from-pair")
      end
    end.new

    xcfg = Struct.new(:recording_studio_pages).new(pair_config)
    app_config = Struct.new(:x).new(xcfg)
    ...
    assert_equal "/from-pair", RecordingStudioPages.configuration.homepage_path
  end
  ...
  def test_load_config_swallow_each_pair_errors
    ...
    xcfg = Struct.new(:recording_studio_pages).new(bad_pair_config)
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { homepage_path: "/ok" }
    ...
    assert_equal "/ok", RecordingStudioPages.configuration.homepage_path
  end
  ...
  def test_load_config_is_noop_without_config_sources
    ...
    assert_equal "/", RecordingStudioPages.configuration.homepage_path
    assert_equal true, RecordingStudioPages.configuration.register_built_in_sections
  end
  ...
  def test_load_config_ignores_non_enumerable_yaml_and_merge_errors
    ...
    xcfg = Struct.new(:recording_studio_pages).new({ homepage_path: "/from-x" })
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      attr_accessor :yaml
    ...
    assert_equal "/from-x", RecordingStudioPages.configuration.homepage_path
  end
  ...
  def test_apply_extension_initializers_register_active_support_on_load_callbacks
    ...
    RecordingStudioPages::Engine.stub(:config, config_stub) do
      find_initializer("recording_studio_pages.apply_model_extensions").block.call
      find_initializer("recording_studio_pages.apply_controller_extensions").block.call
    end
  ...
  def test_model_extension_initializer_skips_abstract_models
    ...
    with_temporary_nested_constant(:ActiveRecord, :Base, active_record_base) do
      RecordingStudioPages::Engine.stub(:apply_model_extensions, ->(model) { applied << model }) do
  ...
  def test_controller_extension_initializer_applies_all_controllers
    ...
    with_temporary_nested_constant(:ActionController, :Base, action_controller_base) do
      RecordingStudioPages::Engine.stub(:apply_controller_extensions, ->(controller) { applied << controller }) do
        to_prepare_blocks.first.call
      end
    end
  ...
  def test_apply_model_extensions_adds_registered_methods_once
    ...
    RecordingStudioPages.configuration.hooks.extend_model(:ExampleRecord) do
      def template_extension_method
        :applied
      end
    end

    RecordingStudioPages::Engine.apply_model_extensions(model_class)
    RecordingStudioPages::Engine.apply_model_extensions(model_class)
  ...
  def test_apply_controller_extensions_matches_demodulized_name
    ...
    RecordingStudioPages.configuration.hooks.extend_controller(:DashboardController) do
      def template_controller_extension
        :applied
      end
    end

    RecordingStudioPages::Engine.apply_controller_extensions(controller_class)
  ...
  def test_apply_extensions_flattens_compacts_and_tracks_identity
    ...
    RecordingStudioPages::Engine.send(:apply_extensions, target, [nil, [extension, extension]])

    assert_equal :generated, target.new.generated_method
    assert_equal true, target.instance_variable_get(:@recording_studio_pages_applied_extensions).compare_by_identity?
  end

  def test_apply_extensions_returns_without_target
    assert_nil RecordingStudioPages::Engine.send(:apply_extensions, nil, [])
  end
  ...
  def test_extension_keys_for_includes_demodulized_name
    ...
    assert_equal expected_keys, RecordingStudioPages::Engine.send(:extension_keys_for, namespaced)
  end
  ...
  def test_extension_keys_for_removes_duplicate_names
    ...
    assert_equal [:ReportsController], RecordingStudioPages::Engine.send(:extension_keys_for, plain)
  end
  ...
  def find_initializer(name)
    RecordingStudioPages::Engine.initializers.find { |initializer| initializer.name == name }
  end
end
