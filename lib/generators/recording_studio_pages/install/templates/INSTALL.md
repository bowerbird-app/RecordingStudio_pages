...
11. Register custom sections in the `:register_sections` hook and custom hero fillings in `:register_ctas`. The engine resets registries on reload.
12. Point Tailwind `@source` at this gem's `app/views` and `app/components`, then run `bin/rails tailwindcss:build`.
13. Keep strict recordable declarations enabled and add `recording_studio_recordable(...)` to every configured recordable before running `RecordingStudio.validate_recordable_declarations!`.
