...
8. Adjust auth, layout, and current actor integration to match your host app. Public pages must use a full-width layout that loads `flat_pack/variables` then `flat_pack/application` then Tailwind, with the host Flatpack theme on `html` (`FlatPack.configuration.default_theme`). Load Flatpack Stimulus so a Menu’s **More** control works on a phone. Do not reuse a sign-in layout (`max-w-md`).
