...
9. Register custom sections in the `:register_sections` hook. The engine resets registries on reload.
10. Point Tailwind `@source` at this gem's `app/views` and `app/components`, then run `bin/rails tailwindcss:build`.
11. Keep strict recordable declarations enabled and add `recording_studio_recordable(...)` to every configured recordable before running `RecordingStudio.validate_recordable_declarations!`.
